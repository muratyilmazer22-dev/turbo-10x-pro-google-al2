...
export function calculateDynamicAHP(
  ...
  const safeResults = Array.isArray(recentRaceResults) ? recentRaceResults : [];

  // Hafıza Bankası Boost
  const hafiza_boost = Math.min(6.0, safeNotes.length * 1.5);

  // 1. AGF PUAN (Tüm İşler Gemileri)
  let agf_puan = 75.0;
  if (safeResults.length > 0) {
    const avgPosition = safeResults.reduce((sum, r) => sum + (r?.position || 4), 0) / safeResults.length;
  ...
  // 3. JOKEY FORM
  let jokey_form = 75.0;
  if (safeNotes.some(n => typeof n === "string" && n.toLowerCase().includes("jokey"))) {
    jokey_form = 85.0;
  }
  ...
  // 5. GALOP GUCU
  let galop_gucu = 75.0;
  if (safeNotes.some(n => typeof n === "string" && n.toLowerCase().includes("galop"))) {
    galop_gucu = 85.0;
  }

  // 6. FORM 6 YARIS
  let form_6yaris = 65.0;
  if (safeResults.length === 6) {
    const wins = safeResults.filter(r => r?.position === 1).length;
    form_6yaris = 50 + (wins * 8);
  }

  // 7. PIST UYUMU
  let pist_uyumu = 72.0;
  if (safeResults.some(r => r?.trackType === "Çim")) {
    pist_uyumu = 80.0;
  }
  ...
  if (safeNotes.some(n => typeof n === "string" && n.toLowerCase().includes("sprint"))) {
    sprint_gucu = 85.0;
  }
  ...
  if (safeResults.length > 0) {
    const lastRace = safeResults[safeResults.length - 1];
    son_kosu_performansi = Math.max(60, 100 - ((lastRace?.position || 4) * 10));
  }
  ...
  if (safeNotes.some(n => typeof n === "string" && n.toLowerCase().includes("stres"))) {
    kafa_sagligi = 65.0;
  }
  ...
  if (safeNotes.some(n => typeof n === "string" && n.match(/KG|DB|SK|OG|HP/))) {
    ozellik_kupu = 85.0;
  }
